//! Ambient access: scene entry and shaping resolve the task's installed contexts.
//!
//! Application code runs inside a task whose change queue and shaping context are installed, so
//! neither has to be named at the call site (ADR 0008). [`Enter`] publishes a scene value into
//! the installed queue; [`AmbientShape`] shapes text with the installed shaper.
//!
...
//! same operation for code that runs inside a task. The traits live in this crate rather than in
//! `massive-shapes` because the task-locals they read are installed above it.
