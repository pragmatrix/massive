...
use crate::AnimationTimeProvider;
...
impl TimeScale {
    pub fn new(context: &dyn AnimationTimeProvider) -> Self {
    ...
    pub fn scale_seconds(&mut self, context: &dyn AnimationTimeProvider) -> f64 {
        self.duration_passed(context).as_secs_f64()
    }

    /// The duration passed since the last update cycle (ZERO if the [`TimeScale`] was just
    /// generated).
    pub fn duration_passed(&mut self, context: &dyn AnimationTimeProvider) -> Duration {
