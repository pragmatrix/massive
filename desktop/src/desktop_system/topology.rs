...
// pub struct DesktopTopology<'a>(&'a dyn LayoutTopology<DesktopTarget>);

// Idea: Unsized coerction (this is interesting, but not in use right now, seems to be overblown)
// #[derive(Deref, DerefMut)]
// pub struct DesktopTopologyImpl<T: LayoutTopology<DesktopTarget> + ?Sized>(T);

// pub type DesktopTopology = DesktopTopologyImpl<dyn LayoutTopology<DesktopTarget>>;

// pub fn desktop_topology<T: LayoutTopology<DesktopTarget> + 'static>(
//     topology: &DesktopTopologyImpl<T>,
// ) -> &DesktopTopology {
//     topology
// }
...
impl OrderedHierarchy<DesktopTarget> {
    pub fn launcher_of_instance(&self, instance_id: InstanceId) -> Option<LaunchProfileId> {
        match self.parent_of(&DesktopTarget::Instance(instance_id)) {
