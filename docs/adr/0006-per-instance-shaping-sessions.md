...
A shaper rule, made API-visible by this design: **a shaper must not outlive the frame cycle it shaped for** — its republish-on-drop is what keeps the renderer's contract exact (below).

### Shaper exclusivity at runtime, not via borrow gating

`shaper()` and `load_font` take `&self`. A shaper exclusively holds its handle's scratch mutex, and `shaper()` acquires it with `try_lock`, so a second shaper on the same handle **panics at the misuse point** (with a message naming the handle and the held scratch) instead of deadlocking on a non-reentrant parking_lot mutex. Shapers on different handles shape in parallel; `load_font` during an open shaper is safe by construction (the manager mutex is mint-time-only and never held while shaping). The trade: a misuse that a compile-time gate would reject now panics at runtime — judged worth it to drop the `&mut` plumbing through every presenter and constructor.
...
- A cosmic shaper, at its start, compares the manager's published snapshot against the one it synced last; on mismatch it swaps its `FontSystem`'s db from the published snapshot and updates its local copy. New fonts are visible on the instance's next shaper — worst case one frame.
...
The renderer's "registry miss is a real bug" stance stays absolute, and the debug assert of `GlyphRun.shaping_engine` against the manager's engine stays. Both survive because: republishing still happens at drop, shapers do not outlive their frame cycle, and drop precedes submission within a task.
...
- **Eager mint-time republish of interned cosmic faces** (publish inside `shape()`). Rejected on grilling review: a shaper is dropped before anything it shaped can be submitted, so the drop-publish window is unreachable; no contract change is needed.
...
- `load_font` mid-run is legal and becomes visible without coordinator knowledge: parley via fontique's version sync, cosmic via epoch-pull at next shaper open.
